...
from mosaic.shapes import Rectangle, Point
from mosaic.util import get_average_color, get_next_near_image, add_colour_filter, \
    calc_gradient_field
from .preprocessing import batch_preprocess, resize_img, to_square_img, open_as_pil_image, square_img
...
def build_mosaic(target_image: PIL.Image.Image,
    ...
    try:
        ...
        # оптимально подобраное число - по алгоритму будет производится поиск полей для укрупнения
        # от ukrup_field_max_size до 3 пиксельных квадратов
        while step > 3:
        ...
        for size in shape_sizes:
            ...
            while x_start_shape <= target_image.width-size:
                ...
                while y_start_shape <= target_image.height-size:
                    ...
                    # если внутри квадрата уже ловили занятую точку,
                    # пропускаем ход и идем к след.квадрату
                    if not is_empty_rect_field:
...
def create_mosaic(img_path: str,
    ...
    :param ukrup_field_max_percent - максимальный размер области которую можно укрупнить (процент от меньшей длины)
    :param not_ukrup_fields -  список областей, в которых не требуется укрупнение. область в формате десятичных долей от x,y..
    :return:
    """
    try:
